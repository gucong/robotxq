...
static void PopPvLine(int nDepth = 0, int vl = 0) {
  ...
  printf("%d %d %d %d", nDepth, vl, (int) (GetTime() - Search2.llTime) / 10, Search2.nAllNodes);
...
void SearchMain(int nDepth) {
  ...
  // 由于 ClearHash() 需要消耗一定时间，所以计时从这以后开始比较合理 ([gc]: 合理你个头)
  ...
  if (Search.bUseBook) {
    ...
    if (nBookMoves > 0) {
      ...
      if (Search.pos.RepStatus(3) == 0) {
        ...
        Search.nTimeUsed = (GetTime() - Search2.llTime) / 10;
  ...
  for (i = 1; i <= nDepth; i ++) {
    ...
    if ((Search2.bPopPv || Search.bDebug) && xb_post)
        printf("%d %d %d %d\n",
               i,
               vl,
               (int) (GetTime() - Search2.llTime) / 10,
